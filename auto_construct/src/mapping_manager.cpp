...
MappingManager::MappingManager() : Node("mapping_manager")
{
  ...
  srv_load_map_ = this->create_service<nav2_msgs::srv::LoadMap>(
    "/sys/load_map",
    std::bind(&MappingManager::handle_load_map, this,
              std::placeholders::_1, std::placeholders::_2),
    rmw_qos_profile_services_default, cb_group_);
  ...
  client_load_map_ = this->create_client<nav2_msgs::srv::LoadMap>(
    "/coverage/load_map",
    rmw_qos_profile_services_default, cb_group_);
...
void MappingManager::handle_load_map(
  ...
  std::shared_ptr<nav2_msgs::srv::LoadMap::Response> res)
{
  ...
  if (res->result == nav2_msgs::srv::LoadMap::Response::RESULT_SUCCESS) {
    RCLCPP_INFO(this->get_logger(), "🗺️  地图加载成功");
  } else {
    RCLCPP_WARN(this->get_logger(), "🗺️  地图加载失败，错误码: %d", res->result);
  }
}
