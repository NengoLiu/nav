...
SaveMap::SaveMap() : Node("save_map_node") {
    ...
    save_srv_ = this->create_service<std_srvs::srv::Trigger>(
        "/mapping/save_map", std::bind(&SaveMap::save_callback, this, _1, _2));
    ...
    // 3. 初始化向底层节点发送指令的客户端
    octomap_reset_client_ = this->create_client<std_srvs::srv::Empty>("/mapping/octomap_server/reset");
    nav2_save_client_ = this->create_client<nav2_msgs::srv::SaveMap>("/nav2/map_saver_server/save_map");
